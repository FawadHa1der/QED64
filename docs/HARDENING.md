...
27. **Frame parsers must locate the header, not assume it.** The worker's stdout interleaves library progress lines with LSP frames. A parser that scans only the first 256 bytes for `Content-Length` wedges on the first such line while the worker keeps answering — misdiagnosed as a runtime stall for hours. The obvious fix (decode the whole buffer per chunk) is O(n²) under a diagnostics storm and allocates megabyte strings per call. The scan is byte-level (`indexOfHeader` in lean.worker.js), pinned by tests/unit/frame-parser.test.ts including multibyte offsets and a 4 MB linear-time case.
