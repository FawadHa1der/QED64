...
// three states, zero timers, zero regexes. The worker owns the document, the
// queue and every header verdict (§2.4); this relay only remembers what it
// needs to re-establish the document on a fresh session (§2.3 data), fails
// requests a death orphaned (§3 row 11), and breaks crash loops. It reads
// exactly four fields of any message: method, id, version, text.
...
export interface RelaySession {
  readonly id: string;
  /** Boot: resolves = BootOk, rejects = BootFailed (§2.3). */
  start(): Promise<void>;
  ...
  /** Detaches first, never emits a death (§2.2 L2). */
  dispose(): void;
}
/** Boot inputs for a replacement session (S4's "Load exact imports"). */
export interface RestartOptions { snapshots?: string[]; warmHeader?: string; packs?: string[] }
...
export class LspRelay {
  ...
  readonly stats = { reboots: 0, userRestarts: 0, workerDeaths: 0, breakerTrips: 0, failedInFlight: 0, staleDeaths: 0 };

  constructor(
    private readonly makeSession: (opts?: RestartOptions) => RelaySession,
    private readonly sink: { status(s: RelayStatus): void },
    /** The one wait in the design — the 1.5 s heap-release settle inside a reboot
     * transition (§2.3, §2.5) — injected so this module never owns a timer (§8 item 9). */
  ...
  fromClient(msg: JsonRpc): void {
    const p = msg.params as { textDocument?: { uri: string; languageId: string; version: number; text: string }; contentChanges?: { text?: string }[] } | undefined;
  ...
  /** Unload (§2.3). */
  unload(): void { this.session.dispose(); }
  ...
  private async boot(s: RelaySession, settle: boolean): Promise<void> {
    ...
    try { await s.start(); } catch { return this.onDied(s, "bootFailed"); }
    if (s !== this.session) return;
    // BootOk: re-establish the document from what THIS relay forwarded, not
    // from the editor (lean4game's editor text is untranslated — §2.2 L3).
  ...
  /** §3 row 11: answer every orphaned request in the same turn — rpc calls get
   * RpcNeedsReconnect (-32900) so the InfoView reconnects and refetches; the rest -32603. */
