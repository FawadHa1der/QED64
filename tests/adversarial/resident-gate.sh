...
# the pairing (manifest, chunks, snapshot index, runtime↔snapshot pairing,
# one boot smoke — refuses with exit 3 before any lane runs, HARDENING #32/#33),
# then run the e2e suite, the editing-latency benchmark and the compiler
# battery on the new pairing, with a browser cool-down between lanes
# (HARDENING #34). Resident is the page's only transport (the pump lanes left
# with it, 2026-09-04). Usage: tests/adversarial/resident-gate.sh
set -u
...
cool
echo "=== e2e ==="; node tests/adversarial/e2e.mjs --url "$URL" --run-dir "$RUN" > work/adversarial/e2e-resident.log 2>&1; echo "e2e-exit=$?"
...
print(f\"e2e: {r['total']-r['failed']}/{r['total']} (infra {r.get('infra',0)}, aborted {r.get('aborted',0)})\")
